...
// ─── IField OUTSIDE component to prevent re-mount on every render ──────────
const IField = ({ label, value, onChange, suffix = "₫" }) => (
...
// ─── HEADCOUNT TAB (outside to prevent re-mount) ───────────────────────────
const HeadcountTab = ({ headcount, result, updateHeadcount }) => {
  const hc = headcount
...
// ─── TECH STACK TAB (outside to prevent re-mount) ─────────────────────────
const TechStackTab = ({ techstack, result, updateTechstack }) => {
  const ts = techstack
...
// ─── MONTHLY P&L TAB (outside to prevent re-mount) ────────────────────────
const MonthlyTab = ({ result, assumptions }) => {
...
// ─── COMPARE TAB (outside to prevent re-mount) ────────────────────────────
const CompareTab = ({ projects, activeId, calcPnLFn, onNewProject }) => {
  if (projects.length < 2) return (
    <div style={S.emptyState}>
      <div style={{ fontSize: 32 }}>⚖️</div>
      <div style={{ fontSize: 16, fontWeight: 700, color: "#e2e8f0" }}>Cần ít nhất 2 dự án để so sánh</div>
      <button style={S.btn("primary")} onClick={onNewProject}>+ Tạo dự án mới</button>
    </div>
  )
  const results = projects.map(p => ({ p, r: calcPnLFn(p) }))
...
export default function PnLApp() {
  ...
  const updateAssumption = useCallback((key, val) => {
    setProjects(prev => {
      const updated = prev.map(p => p.id === activeId ? { ...p, assumptions: { ...p.assumptions, [key]: val } } : p)
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(updated)) } catch {}
      return updated
    })
  }, [activeId])

  const updateHeadcount = useCallback((newHc) => {
    setProjects(prev => {
      const updated = prev.map(p => p.id === activeId ? { ...p, headcount: newHc } : p)
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(updated)) } catch {}
      return updated
    })
  }, [activeId])

  const updateTechstack = useCallback((newTs) => {
    setProjects(prev => {
      const updated = prev.map(p => p.id === activeId ? { ...p, techstack: newTs } : p)
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(updated)) } catch {}
      return updated
    })
  }, [activeId])
  ...
  const calcPnLForProject = useCallback((p) => calcPnL(p.assumptions, p.headcount, p.techstack), [])
  ...
  // Inputs tab — dùng IField với value/onChange thay vì key lookup
  const InputsTab = () => {
    if (!activeProject) return null
    const a = activeProject.assumptions
    const F = ({ label, k, suffix = "₫" }) => (
      <IField label={label} value={a[k]} onChange={v => updateAssumption(k, v)} suffix={suffix} />
    )
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
        <div style={S.card()}>
          ...
          <F label="Giá Basic" k="priceBasic" />
          <F label="Giá Pro" k="pricePro" />
          <F label="Giá Enterprise" k="priceEnterprise" />
          <F label="Annual Discount (%)" k="annualDiscount" suffix="%" />
          <F label="Setup blended (₫/KH mới)" k="setupBlended" />
          <span style={S.secLabel}>Customer mix</span>
          <F label="Khách đầu T1" k="startingCustomers" suffix="KH" />
          <F label="Tăng trưởng KH/tháng (%)" k="monthlyGrowthRate" suffix="%" />
          <F label="Churn rate/tháng (%)" k="monthlyChurnRate" suffix="%" />
          <F label="% KH Monthly plan" k="pctMonthly" suffix="%" />
          <F label="% KH Enterprise tier" k="pctEnterprise" suffix="%" />
          <F label="% KH Pro tier" k="pctPro" suffix="%" />
          <F label="Avg seats/KH" k="avgUsersPerCustomer" suffix="seat" />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={S.card()}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#f1f5f9", marginBottom: 10 }}>🏭 COGS (per-customer)</div>
            <F label="Hosting base (₫/tháng)" k="hostingBase" />
            <F label="Hosting/KH (₫/tháng)" k="hostingPerCustomer" />
            <F label="Support cost/KH (₫/tháng)" k="supportPerCustomer" />
            <F label="Payment gateway (% Rev)" k="paymentGatewayPct" suffix="%" />
          ...
          <div style={S.card()}>
            ...
            <F label="Sales commission (% New MRR)" k="salesCommissionPct" suffix="%" />
            <F label="Marketing budget/tháng" k="marketingBudget" />
            <F label="G&A – Rent, Admin, Legal" k="gaRent" />
            <F label="Depreciation/Amortization" k="depreciation" />
          ...
          <div style={S.card()}>
            ...
            <F label="CIT rate (%)" k="citRate" suffix="%" />
            <F label="VAT (%) if applicable" k="vatRate" suffix="%" />
          </div>
        </div>
      </div>
    )
    ...
    <div style={{ ...S.app, display: "flex", height: "100vh" }}>
      ...
      <div style={S.main}>
        ...
        <div style={S.content}>
          ...
          {activeProject && tab === "inputs" && <InputsTab />}

          {activeProject && tab === "headcount" && (
            <HeadcountTab
              headcount={activeProject.headcount}
              result={result}
              updateHeadcount={updateHeadcount}
            />
          )}

          {activeProject && tab === "techstack" && (
            <TechStackTab
          ...
          {activeProject && tab === "monthly" && (
            <MonthlyTab result={result} assumptions={activeProject.assumptions} />
          )}
